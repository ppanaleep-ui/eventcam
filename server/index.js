...
async function main() {
  ...
  app.use(authOptional); // attaches req.user when a valid session cookie exists

  if (config.isProd()) {
    if (!config.jwtSecret) console.warn('⚠  JWT_SECRET is not set — set it (and keep it stable) in production.');
    if (!config.ownerEmail) console.warn('⚠  OWNER_EMAIL is not set — no account will be auto-approved as owner.');
  }

...
// serves public files that existed at server startup — runtime uploads there
// would 404. Files are streamed back through /api/files/[...path] instead.
...
/**
 * Persists an uploaded image file and returns its public URL
 * (e.g. /api/files/wishes/xxx.jpg). `subdir` groups files.
 */
export async function saveImage(file: File, subdir: string): Promise<string> {
  ...
  if (file.size > MAX_BYTES) {
    throw new UploadError("File is too large (max 12MB)");
  }
